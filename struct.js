...
var GamePlayer = function()
{
    ...
    this.DoubleBets = function()
    {
        ...
        if (this.BetList.length > 0)
        {
            ...
            for (var I = 0; I < this.BetList.length; I++)
            {
                ...
                var BetValue = BetItem[7] * 2;
                var MaxVal = 10000;
                MaxVal = Math.floor(MaxVal / Math.abs(1));
                MaxVal = MaxVal * Math.abs(1);
                ...
                BetListNew.push([BetItem[0], BetItem[1], BetItem[2], BetItem[3], BetItem[4], BetItem[5], BetItem[6], BetValue]);
            }
            this.BetList = BetListNew;
            this.AmountT = 0;
            for (var I = 0; I < this.BetList.length; I++)
            {
    ...
    this.RestoreBetsAuto = function(ClrFactor)
    {
        this.WheelNeighLastS = -1;
        if (this.BetList.length == 0 && this.BetList0.length > 0)
        {
            if (ClrFactor == null || isNaN(ClrFactor) || ClrFactor <= 0)
            {
                ClrFactor = 1;
            }
    ...
    this.Account = function(N, Prob)
    {
        ...
        for (var I = 0; I < this.BetList.length; I++)
        {
            ...
            if (ToRemove)
            {
                ...
                {
                    this.AmountT -= BetVal;
                    // Las apuestas no se eliminan al perder
                    // this.BetList.splice(I, 1);
                    // I--;
                }
            }
