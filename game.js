...
function TableClick(X, Y, Btn)
{
    ...
    for (var I = 0; I < ButtonList.length; I++)
    {
        if ((X > ButtonList[I][1]) && (Y > ButtonList[I][2]) && (X < ButtonList[I][3]) && (Y < ButtonList[I][4]) && (!Spinning))
        {
            ...
            if ((!PlayerSelected()) && (ButtonList[I][0] >= 3 && ButtonList[I][0] <= 8 || ButtonList[I][0] == 9 || ButtonList[I][0] == 12))
            {
                DisplayHelp();
            }
            ...
            if (PlayerSelected())
            {
                switch (ButtonList[I][0])
                {
                    ...
                    case 13:
                        ...
                        StatePlayerSave(false);
                        // Auto spin after doubling bets
                        if (GamePlayer_[GamePlayer_N].BetList.length > 0)
                        {
                            PaintGame();
    ...
    // Click on Auto-Spin and Martingala switches
    if (!Spinning && (X >= AutoSpinButtonArea[0]) && (Y >= AutoSpinButtonArea[1]) && (X <= AutoSpinButtonArea[2]) && (Y <= AutoSpinButtonArea[3]))
    {
        // Auto-Spin button clicked
        if (AutoSpinMode == 1)
        {
            // Stop auto-spin
            StopAutoSpin();
        ...
        else if (AutoSpinMode == 2)
        {
            // Switch from Martingala to Auto-Spin
            StopMartingala();
            AutoSpinMode = 1;
            StartAutoSpin();
        }
    ...
    if (!Spinning && (X >= MartingalaButtonArea[0]) && (Y >= MartingalaButtonArea[1]) && (X <= MartingalaButtonArea[2]) && (Y <= MartingalaButtonArea[3]))
    {
        // Martingala button clicked
        if (AutoSpinMode == 2)
        {
            // Stop martingala
            StopMartingala();
        ...
        else if (AutoSpinMode == 1)
        {
            // Switch from Auto-Spin to Martingala
            StopAutoSpin();
            AutoSpinMode = 2;
            StartMartingala();
        }
        else if (AutoSpinMode == 3)
        {
            // Switch from Martingala X2 to Martingala
            StopMartingalaX2();
            AutoSpinMode = 2;
            StartMartingala();
        }
    ...
    if (!Spinning && (X >= MartingalaX2ButtonArea[0]) && (Y >= MartingalaX2ButtonArea[1]) && (X <= MartingalaX2ButtonArea[2]) && (Y <= MartingalaX2ButtonArea[3]))
    {
        // Martingala X2 button clicked
        if (AutoSpinMode == 3)
        {
            // Stop martingala X2
            StopMartingalaX2();
        ...
        {
            // Switch from Auto-Spin to Martingala X2
            StopAutoSpin();
            AutoSpinMode = 3;
            StartMartingalaX2();
        }
        ...
        {
            // Switch from Martingala to Martingala X2
            StopMartingala();
            AutoSpinMode = 3;
            StartMartingalaX2();
        }
