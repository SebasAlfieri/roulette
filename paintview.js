...
var AutoSpinButtonArea = [0, 0, 0, 0];
var MartingalaButtonArea = [0, 0, 0, 0];
var MartingalaX2ButtonArea = [0, 0, 0, 0];
...
function PaintClear()
{
    ...
    // Posición 0: Previous player
    DrawButton(1, (TableW  *  0 / BtnCount) + BtnOffset, BtnSize, 1, "<< player");
    
    // Posición 1: Next player
    DrawButton(2, (TableW  *  1 / BtnCount) + BtnOffset, BtnSize, 1, "player>>");

    // Posición 2: Chip Value Down
    DrawButton(3, (TableW  *  2 / BtnCount) + BtnOffset, BtnSize, 1, "ChipValue -");
    
    // Posición 3: Chip Value Up
    DrawButton(4, (TableW  *  3 / BtnCount) + BtnOffset, BtnSize, 1, "ChipValue +");

    // Posición 4: Factor Down
    DrawButton(5, (TableW  *  4 / BtnCount) + BtnOffset, BtnSize, 1, "F-");
    
    // Posición 5: Factor Up
    DrawButton(6, (TableW  *  5 / BtnCount) + BtnOffset, BtnSize, 1, "F+");

    // Posición 6: Toggle Bet Sign
    DrawButton(7, (TableW  *  6 / BtnCount) + BtnOffset, BtnSize, 1, "+/-");

    // Posición 7: Clear Bets
    DrawButton(8, (TableW  *  7 / BtnCount) + BtnOffset, BtnSize, 1, "CLEAR");

    // Posición 8: Add Player
    DrawButton(9, (TableW  *  8 / BtnCount) + BtnOffset, BtnSize, 1, "ADD PLAYER");

    // Posición 9: Spin Mode (switches dynamically)
    switch (SpinMode)
    {
        case 0:
            DrawButton(12, (TableW * 9 / BtnCount) + BtnOffset, BtnSize, 1, "STANDARD");
            break;
        case 1:
...
function PaintAutoSpinSwitch()
{
    // Draw auto-spin and martingala switch panel in top-right corner (vertical layout)
    var SwitchWidth = TableW * 0.15;
    var SwitchHeight = ViewMargin * 2.5;
    var SwitchX = TableW - SwitchWidth - ViewMargin * 0.5;
    var SwitchY = ViewMargin * 2;
    var SwitchPadding = ViewMargin * 0.08;
    var ButtonWidth = SwitchWidth - SwitchPadding * 2;
    var ButtonHeight = (SwitchHeight - SwitchPadding * 4) / 3;
    
    // Background panel
    SvgAddRect(SvgBack, SwitchX, SwitchY, SwitchWidth, SwitchHeight, "", "rgb(50, 50, 50)");
    SvgAddRect(SvgBack, SwitchX, SwitchY, SwitchWidth, SwitchHeight, "rgb(150, 150, 150)", "");
    
    // Auto-Spin button
    var AutoSpinColor = AutoSpinMode == 1 ? "rgb(0, 200, 0)" : "rgb(100, 100, 100)";
    SvgAddRect(SvgBack, SwitchX + SwitchPadding, SwitchY + SwitchPadding, ButtonWidth, ButtonHeight, "rgb(0, 0, 0)", AutoSpinColor);
    SvgAddText(SvgBack, SwitchX + SwitchPadding + ButtonWidth / 2, SwitchY + SwitchPadding + ButtonHeight / 2, "AUTO", "rgb(0, 0, 0)");
    AutoSpinButtonArea = [SwitchX + SwitchPadding, SwitchY + SwitchPadding, SwitchX + SwitchPadding + ButtonWidth, SwitchY + SwitchPadding + ButtonHeight];
    
    // Martingala button (below AUTO button)
    var MartingalaColor = AutoSpinMode == 2 ? "rgb(200, 0, 0)" : "rgb(100, 100, 100)";
    var MartingalaY = SwitchY + SwitchPadding + ButtonHeight + SwitchPadding;
    SvgAddRect(SvgBack, SwitchX + SwitchPadding, MartingalaY, ButtonWidth, ButtonHeight, "rgb(0, 0, 0)", MartingalaColor);
    SvgAddText(SvgBack, SwitchX + SwitchPadding + ButtonWidth / 2, MartingalaY + ButtonHeight / 2, "MARTINGALA", "rgb(0, 0, 0)");
    MartingalaButtonArea = [SwitchX + SwitchPadding, MartingalaY, SwitchX + SwitchPadding + ButtonWidth, MartingalaY + ButtonHeight];
    
    // Martingala X2 button (below Martingala button)
    var MartingalaX2Color = AutoSpinMode == 3 ? "rgb(255, 100, 0)" : "rgb(100, 100, 100)";
    var MartingalaX2Y = MartingalaY + ButtonHeight + SwitchPadding;
    SvgAddRect(SvgBack, SwitchX + SwitchPadding, MartingalaX2Y, ButtonWidth, ButtonHeight, "rgb(0, 0, 0)", MartingalaX2Color);
    SvgAddText(SvgBack, SwitchX + SwitchPadding + ButtonWidth / 2, MartingalaX2Y + ButtonHeight / 2, "MART X2", "rgb(0, 0, 0)");
    MartingalaX2ButtonArea = [SwitchX + SwitchPadding, MartingalaX2Y, SwitchX + SwitchPadding + ButtonWidth, MartingalaX2Y + ButtonHeight];
    
    // Display counters if in auto/martingala mode
    if (AutoSpinMode >= 1)
    {
        var CounterY = MartingalaX2Y + ButtonHeight + SwitchPadding * 2;
        var CounterFontSize = ButtonHeight * 0.4;
        var TextColor = "rgb(200, 200, 200)";
        
        // Display spin count
        var SpinText = "Spins: " + AutoMartingalaSpinCount;
        SvgAddText(SvgBack, SwitchX + SwitchPadding + ButtonWidth / 2, CounterY, SpinText, TextColor);
        
        // Display negative balance count
        var NegativeText = "Negatives: " + AutoMartingalaNegativeCount;
        SvgAddText(SvgBack, SwitchX + SwitchPadding + ButtonWidth / 2, CounterY + CounterFontSize * 1.5, NegativeText, TextColor);
    }
}
